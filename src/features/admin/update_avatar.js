...
import { URL_UPDATE_ADMIN_AVATAR } from '../../api/api';
import Axios from 'axios';

import { updateAdminAvatar } from './admin_slice';
...
const UpdateAvatarAdmin = props => {
    let { adminId, avatar } = props
    ...
    const handleUpdate = async (e) => {
        ...
        try{
            ...
            let response = await Axios.put(URL_UPDATE_ADMIN_AVATAR(adminId),data,{
                headers : {
                    token : localStorage.getItem("token")
                }
            })
            dispatch(updateAdminAvatar({avatar : response.data.data.avatar}))
            reset()
            setSuccess()
            setTimeout(() =>{
