...
import { URL_UPDATE_MERCHANT_AVATAR } from '../../api/api';
import Axios from 'axios';

import { updateMerchantAvatar } from './merchant_slice';
...
const UpdateAvatarMerchant = props => {
    let { merchantId, avatar } = props
    ...
    useEffect(() => {
        console.log(`receive avatar : ${avatar}`)
        if(imgInput.current) {
            console.log(`setting up : image ${avatar}`)
            imgInput.current.src = avatar
        }
    },[avatar])
    ...
    const handleUpdate = async (e) => {
        ...
        try{
            ...
            let response = await Axios.put(URL_UPDATE_MERCHANT_AVATAR(merchantId),data,{
                headers : {
                    token : localStorage.getItem("token")
                }
            })
            
            dispatch(updateMerchantAvatar({avatar : response.data.data.avatar}))
            reset()
            setSuccess()
            setTimeout(() =>{
...
export default UpdateAvatarMerchant;